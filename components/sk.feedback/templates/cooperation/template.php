...
<?=bitrix_sessid_post()?>
		<span class="popup__title">Заявка на<br>сотрудничество</span>
<?
...
?>
		<input class="popup__input"<?=(in_array("NAME", $arParams["REQUIRED_FIELDS"]) ? ' required="required"' : '')?> title="ФИО" placeholder="Фамилия Имя Отчество" type="text" name="name" value="<?=(empty($arResult["ERROR_MESSAGE"]) ? '' : $_POST["name"])?>" size="5" />
		...
		<input class="popup__input"<?=(in_array("FIELD1", $arParams["REQUIRED_FIELDS"]) ? ' required="required"' : '')?> title="Название организации" placeholder="Название организации" type="text" name="field1" value="<?=(empty($arResult["ERROR_MESSAGE"]) ? '' : $_POST["field1"])?>" size="5" />
		<textarea class="popup__input popup__message"<?=(in_array("MESSAGE", $arParams["REQUIRED_FIELDS"]) ? ' required="required"' : '')?> title="Вид деятельности" placeholder="Вид деятельности" name="message"><?=(empty($arResult["ERROR_MESSAGE"]) ? '' : $_POST["message"])?></textarea>
